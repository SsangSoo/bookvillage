...
public class BorrowQueryServiceImpl implements BorrowQueryService {
    ...
    @Override
    public BorrowResponse retrieveBorrow(Long borrowId, Long memberId) {

        Borrow retrieveBorrow = borrowRepository.findById(borrowId).orElseThrow(() -> new BusinessException(NOT_EXIST_CONTENT));
        ...
        if(retrieveBorrow.isDeleteValid() && member.isGhostOrMember()) { // 삭제되었는데, 멤버 혹은 유령 회원이라면
